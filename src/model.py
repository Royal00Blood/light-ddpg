...
from tensorflow.python.keras.utils.vis_utils import *
...
def ActorNetwork(num_states=24, num_actions=2, action_high=1):
    # Initialize weights between -3e-3 and 3-e3
    last_init = tf.random_normal_initializer(stddev=0.0005)
    inputs = tf.keras.layers.Input(shape=(num_states, 1), dtype=tf.float32)
    inputs = tf.keras.layers.LSTM(num_states)(inputs)
    out = tf.keras.layers.Dense(1024, activation=tf.nn.leaky_relu, kernel_initializer=KERNEL_INITIALIZER)(inputs)
    out = tf.keras.layers.Dense(1024, activation=tf.nn.leaky_relu, kernel_initializer=KERNEL_INITIALIZER)(out)
    outin = tf.keras.layers.Dense(1024, activation=tf.nn.leaky_relu, kernel_initializer=KERNEL_INITIALIZER)(out)

    outV = tf.keras.layers.Dense(1024, activation=tf.nn.leaky_relu, kernel_initializer=KERNEL_INITIALIZER)(outin)
    outV = tf.keras.layers.Dense(1024, activation=tf.nn.leaky_relu, kernel_initializer=KERNEL_INITIALIZER)(outV)
    outV = tf.keras.layers.Dense(512, activation=tf.nn.leaky_relu, kernel_initializer=KERNEL_INITIALIZER)(outV)
    outputsV = tf.keras.layers.Dense(1, activation="tanh", kernel_initializer=last_init)(outV) * action_high

    outW = tf.keras.layers.Dense(1024, activation=tf.nn.leaky_relu, kernel_initializer=KERNEL_INITIALIZER)(outin)
    outW = tf.keras.layers.Dense(1024, activation=tf.nn.leaky_relu, kernel_initializer=KERNEL_INITIALIZER)(outW)
    outW = tf.keras.layers.Dense(512, activation=tf.nn.leaky_relu, kernel_initializer=KERNEL_INITIALIZER)(outW)
    outputsW = tf.keras.layers.Dense(1, activation="tanh", kernel_initializer=last_init)(outW) * action_high

    outputs = tf.keras.layers.concatenate([outputsV, outputsW], axis=1)

    model = tf.keras.Model(inputs, outputs)

    # tf.keras.utils.plot_model(model_1, to_file = 'model_1.png', show_shapes = False)

    return model


def CriticNetwork(num_states=24, num_actions=2, action_high=1):
    """
    ...
    state_out = tf.keras.layers.Dense(1024, activation=tf.nn.leaky_relu, kernel_initializer=KERNEL_INITIALIZER)(
        state_input)
    ...
    action_out = tf.keras.layers.Dense(1024, activation=tf.nn.leaky_relu, kernel_initializer=KERNEL_INITIALIZER)(
    ...
    outs = tf.keras.layers.Dense(512, activation=tf.nn.leaky_relu, kernel_initializer=KERNEL_INITIALIZER)(added)
    ...
    # image = 'model_2.png'
    # tf.python.keras.utils.vis_utils.plot_model(model, to_file=image, show_shapes=True)
    return model
...
class Brain:  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(
        ...
        # optimizers
        self.critic_optimizer = tfa.optimizers.RectifiedAdam(CRITIC_LR, amsgrad=True)
        self.actor_optimizer = tfa.optimizers.RectifiedAdam(ACTOR_LR, amsgrad=True)
